...
from .utils import VisionRotaryEmbeddingFast, LayerNorm, PatchDropout
...
class EVA_02(nn.Module):
    def __init__(self, num_stage=3, checkpoint_start_stage=-1,
                 img_size=448, patch_size=14, in_chans=3, num_classes=1000, embed_dim=1024, depth=24,
                 num_heads=16, mlp_ratio=4*2/3, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(LayerNorm, eps=1e-6), init_values=None, use_abs_pos_emb=True,
                 use_rel_pos_bias=False, use_shared_rel_pos_bias=False, use_decoupled_rel_pos_bias=False,
                 use_mean_pooling=True, init_scale=0.001, grad_checkpointing=False, stop_grad_conv1=True,
                 postnorm=False, subln=True, xattn=True, swiglu=False, naiveswiglu=True, rope=True, pt_hw_seq_len=16, intp_freq=True,):
        super().__init__()
