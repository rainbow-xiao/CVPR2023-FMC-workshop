...
from utils import *
from dataset import People_cl_Dataset
...
def val_epoch(model, valid_loader, criterion):
    ...
    with torch.no_grad():
        for (images, labels) in bar:
            ...
            preds = model(images)
            loss = criterion(preds, labels)
            top1_acc = accuracy(preds, labels.argmax(dim=1))[0]
...
def main(config):
    ...
    # df = df.sample(10000)
    dataset_train = People_cl_Dataset(df, args.fold, 'train', config.tag, config.MODEL.img_size)
    dataset_valid = People_cl_Dataset(df, args.fold, 'valid', config.tag, config.MODEL.img_size)
...
#     model._set_static_graph()
    ...
    for epoch in range(1, args.epochs):
        ...
        if args.local_rank==0:
            ...
            if top1_acc > best_top1_acc and epoch>=8:
...
if __name__ == '__main__':
    ...
    if 'train_val_peoples_code_fold_0.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-0'
        config.tag = '0'
    elif 'train_val_peoples_code_fold_1.' in args.csv_dir:
        config.MODEL.num_classes = 3
        config.MODEL.output_dir = config.MODEL.output_dir+'-1'
        config.tag = '1'
    elif 'train_val_peoples_code_fold_2.' in args.csv_dir:
        config.MODEL.num_classes = 3
        config.MODEL.output_dir = config.MODEL.output_dir+'-2'
        config.tag = '2'
    elif 'train_val_peoples_code_fold_3.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-3'
        config.tag = '3'
    elif 'train_val_peoples_code_fold_4.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-4'
        config.tag = '4'
    elif 'train_val_peoples_code_fold_5.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-5'
        config.tag = '5'
    elif 'train_val_peoples_code_fold_6.' in args.csv_dir:
        config.MODEL.num_classes = 3
        config.MODEL.output_dir = config.MODEL.output_dir+'-6'
        config.tag = '6'
    elif 'train_val_peoples_code_fold_7.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-7'
        config.tag = '7'
    elif 'train_val_peoples_code_fold_8.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-8'
        config.tag = '8'
    elif 'train_val_peoples_code_fold_9.' in args.csv_dir:
        config.MODEL.num_classes = 5
        config.MODEL.output_dir = config.MODEL.output_dir+'-9'
        config.tag = '9'
    elif 'train_val_peoples_code_fold_10.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-10'
        config.tag = '10'
    elif 'train_val_peoples_code_fold_11.' in args.csv_dir:
        config.MODEL.num_classes = 2
        config.MODEL.output_dir = config.MODEL.output_dir+'-11'
        config.tag = '11'
    config.freeze()
